...
class StatFormulasTest < MiniTest::Unit::TestCase
  def test_field_goal_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :attempted_field_goals => 22, :made_field_goals => 7
    assert_in_delta(0.318, gl.field_goal_percentage , 0.01)
  end

  def test_free_throw_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :attempted_free_throws => 22, :made_free_throws => 7
    assert_in_delta(0.318, gl.free_throw_percentage , 0.01)
  end

  def test_threes_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :attempted_threes => 22, :made_threes => 7
    assert_in_delta(0.318, gl.threes_percentage , 0.01)
  end

  def test_assist_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :assists => 1, :team_field_goals => 8, :minutes => 24, :team_minutes => 240, :made_field_goals => 0
    assert_in_delta(25, gl.assist_percentage , 0.01)
  end

  def test_block_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :blocks => 1, :opponent_attempted_field_goals => 9, :opponent_attempted_threes => 1, :minutes => 24, :team_minutes => 240
  ...
  def test_effective_field_goal_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :made_field_goals => 1, :attempted_field_goals => 4, :made_threes => 1
    assert_in_delta(0.375, gl.effective_field_goal_percentage , 0.01)
  end

  def test_game_score
    gl = LineTypeFactory.get("2013", :game_line).new :points => 3, :made_field_goals => 1, :attempted_field_goals => 2, :attempted_free_throws => 2, :made_free_throws => 1, :offensive_rebounds => 1, :defensive_rebounds => 1, :steals => 1, :assists =>1, :blocks => 1, :personal_fouls => 1, :turnovers => 1
  ...
  def test_turnover_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :turnovers => 5, :attempted_field_goals => 10, :attempted_free_throws => 10
    assert_in_delta(26, gl.turnover_percentage, 1)
  end

  def test_true_shooting_percentage
    gl = LineTypeFactory.get("2013", :game_line).new :points => 1, :attempted_field_goals => 1, :attempted_free_throws => 1
    assert_in_delta(0.347, gl.true_shooting_percentage, 0.01)
  end

  def test_usage
    gl = LineTypeFactory.get("2013", :game_line).new :attempted_field_goals => 1, :attempted_free_throws => 1, :turnovers => 1, :team_minutes => 240, :minutes => 24, :team_attempted_field_goals => 80, :team_attempted_free_throws => 20, :team_turnovers => 14
