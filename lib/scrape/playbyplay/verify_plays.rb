  ...
  class VerifyPlays
    ...
    def self.verify_team_tallies(team, game_date)
      ...
      Nba::TalleableStatistics.each do |statistic|
        ...
        File.open("playbyplay_errors_#{DateTime.now.strftime('%Y%m%d')}.txt", "a") do |f|
          f.write "results match for #{method_name}. #{team} on #{game_date} #{tally} #{team_total.send(statistic)}\n"
        end unless tally == team_total.send(statistic)
      end
    end
  end
end
