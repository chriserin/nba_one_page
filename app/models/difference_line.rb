class DifferenceLine < GameLine
  include Difference

  after_initialize do
    @called_count = 0
    redefine_formulas
  end

  (statistic_fields - ["minutes", "games_started", "games"]).each do |stat_field|
    field stat_field, type: Array

    define_method "#{stat_field}" do
      if caller.find {|s| s =~ /stat_formulas/}
        result = attributes[stat_field.to_s][@called_count % 2]
        return result
      else
        attributes[stat_field].first - attributes[stat_field].second
      end
    end
  end

  def +(right_side_line)
    throw Exception.new "cannot add game line to difference line" if right_side_line.class == GameLine
    result = DifferenceLine.new
    right_side_line = right_side_line || DifferenceLine.new

    result[:games] = stat(:games) + right_side_line.stat(:games)

    copy_fields(result, :line_name, :team, :is_total, :is_opponent_total, :is_subtotal, :is_difference_total)

    (GameLine.statistic_fields - ["games", "minutes", "games_started"]).each do |statistic|
      result[statistic] = stat(statistic).first + right_side_line.stat(statistic).first, stat(statistic).second + right_side_line.stat(statistic).second
    end

    return result
  end
end
